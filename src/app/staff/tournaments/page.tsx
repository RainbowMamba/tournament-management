...
import { getStaffTournaments } from "@/lib/actions/tournament";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Trophy, Users, Calendar, MapPin, Search } from "lucide-react";
import { StaffTournamentSearch } from "./search";
...
export default async function StaffTournamentsPage({ searchParams }: Props) {
  const { search } = await searchParams;
  const tournaments = await getStaffTournaments(search);
  ...
  return (
    <div className="container mx-auto px-4 md:px-6 py-8">
      <div className="flex flex-col gap-6 mb-8">
        ...
        {/* Search */}
        <StaffTournamentSearch initialSearch={search} />
      </div>
        ...
        <>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {tournaments.map((tournament) => (
              <Link key={tournament.id} href={`/staff/tournaments/${tournament.id}`}>
