...
export function DrawView({ tournament, activeStage, onStageChange }: Props) {
  ...
  function getGroupStandings(groupId: string) {
    ...
    // Map standings back to include team object and calculate points
    return standings.map((standing) => {
      ...
      return {
        ...
        points: standing.wins * 2, // 2 points per win
      };
    });
  }
                  ...
                  <div className="space-y-2">
                    {getGroupMatches(selectedGroup).map((match) => {
                      ...
                      const matchContent = (
                        ...
                        >
                          {/* Hover overlay for matches with details */}
                          {hasDetails && (
...
const MATCH_CARD_HEIGHT = 130; // Height of each match card
const MATCH_CARD_WIDTH = 200; // Width of each match card
const FIRST_ROUND_GAP = 20; // Gap between matches in first round
const CONNECTOR_WIDTH = 32; // Width of connector area between rounds
const HEADER_HEIGHT = 44; // Height of round header
...
function BracketView({ rounds, t, tCommon, getPlaceholderLabel }: BracketViewProps) {
      ...
      <Card className="border-dashed">
        <CardContent className="flex flex-col items-center justify-center py-16">
          ...
          <p className="text-muted-foreground text-center max-w-sm">
            {t('mainDrawNotStartedDescription')}
          </p>
        </CardContent>
      </Card>
    );
  ...
  // Calculate match positions for each round
  // Each match in round N is centered between two matches from round N-1
  ...
  const getMatchYPositions = (roundIndex: number, matchCount: number): number[] => {
    ...
    // Later rounds: center between the two feeder matches
    ...
    return Array.from({ length: matchCount }, (_, i) => {
      ...
      // Center of the current match = midpoint between centers of two previous matches
              ...
              <div key={round} className="flex flex-col" style={{ width: `${columnWidth}px` }}>
                ...
                >
                  {matches.map((match, matchIndex) => {
                    ...
                    return (
                      <div key={match.id}>
                        {/* Match Card - wrapped with dialog if has details */}
                        {hasDetails ? (
                        ...
                        {!isLastRound && (
                          <>
                            ...
                            {/* Vertical line and horizontal to next round (only for even matches) */}
                            {matchIndex % 2 === 0 && matchIndex + 1 < matches.length && (
                              <>
                                ...
                                {/* Horizontal line from vertical to next round match */}
                                <div 
                                  className="absolute bg-border"
                                  style={{
